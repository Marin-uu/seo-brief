...
Collect — the keyword is sent to the SERP API, which returns the top results, People Also Ask, and related searches.
Crawl — the top pages are fetched and their heading structure (H1/H2/H3) and word count are extracted.
Generate — the collected data is sent to Claude, which returns the structured brief as JSON.
Tech stack
   Layer Technology     Frontend / Backend Next.js (App Router)   SERP data Serper.dev   Crawling Jina AI Reader   AI Claude (Anthropic API), model claude-sonnet-4-6   Hosting Vercel   Getting started (local)
Prerequisites
...
cd seo-brief
npm installCreate a file named .env.local in the project root with your API keys:
SERPER_API_KEY=your_serper_key
ANTHROPIC_API_KEY=your_anthropic_key
JINA_API_KEY=your_jina_keyThen run the dev server:
npm run devOpen http://localhost:3000.
Environment variables
   Variable Required Where to get it     SERPER_API_KEY Yes serper.dev (free starter credits)   ANTHROPIC_API_KEY Yes console.anthropic.com (pay-as-you-go; separate from a Claude subscription)   JINA_API_KEY Optional jina.ai/reader (raises rate limits; the reader also works without a key)   .env.local is gitignored — never commit your keys.
Deployment (Vercel)

Push the repo to GitHub.
Import it in Vercel.
Add the same three environment variables in Project Settings → Environment Variables.
Deploy.
...
src/
  ...
  lib/
    ...
    brief.js             Builds the prompt and calls ClaudeNotes & limits
...
Built with help from Claude (Anthropic).seo-brief-tau.vercel.appCreate Next AppGenerated by create next app